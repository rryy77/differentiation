...
/** 数値が簡単な分数に近いとき "1/3", "8/3", "-2/5" のように分数文字列を返す。そうでなければ null */
function toFractionString(n: number): string | null {
  const tol = 1e-5;
  for (let den = 2; den <= 24; den++) {
    for (let num = -30; num <= 30; num++) {
      if (Math.abs(n - num / den) < tol) {
        const g = gcd(num, den);
        const n2 = num / g;
        const d2 = den / g;
...
/** 表示用：分数で表せる場合は分数（1/3, 8/3）、整数ならそのまま、それ以外は小数点第2位 */
function formatCoeff(n: number): string {
  const frac = toFractionString(n);
...
function getXPower(node: math.MathNode): number | null {
  ...
  if (node.type === "OperatorNode") {
    ...
    if (
      op === "^" &&
      args.length === 2 &&
      (args[0] as math.SymbolNode).name === "x"
    ) {
...
/** 項を人間向け表示（x^2 → x²）。係数は分数で表せる場合は分数（2/3 など）、それ以外は小数 */
function termToDisplay(coef: number, power: number): string {
  const cStr = formatCoeff(coef);
...
function termsToExprDisplay(terms: PolyTerm[]): string {
  ...
  return terms
    .map(
      ({ coef, power }) => (coef >= 0 ? "+" : "") + termToDisplay(coef, power)
    )
    .join(" ")
    .replace(/^\+/, "")
    .replace(/\s\+\s-/g, " - ")
  ...
  try {
    ...
    if (atPoint != null && typeof atPoint === "number") {
      try {
        ...
        steps.push(
          `x = ${atPoint} を代入: f'(${atPoint}) = ${formatCoeff(valueAtPoint)}`
        );
      } catch {
        // 代入に失敗しても導関数までは答える
      }
...
function solveFindQuadratic(
  conditions: QuadraticCondition[]
): { expr: string; steps: string[] } | null {
  ...
  try {
    const x = math.lusolve(
      A as unknown as math.Matrix,
      bVec as unknown as math.Matrix
    ) as unknown;
    const arr = (Array.isArray(x) ? x : (x as math.Matrix).toArray()) as
      | number[][]
      | number[];
  ...
  const terms: string[] = [];
  if (a !== 0)
    terms.push(a === 1 ? "x²" : a === -1 ? "-x²" : `${formatCoeff(a)}x²`);
  if (bCoef !== 0)
    terms.push(
      bCoef === 1
        ? "x"
...
function solveFindCubic(
  ...
  try {
    const x = math.lusolve(
      rows as unknown as math.Matrix,
      rhs as unknown as math.Matrix
    ) as unknown;
    const arr = (Array.isArray(x) ? x : (x as math.Matrix).toArray()) as
      | number[][]
      | number[];
  ...
  if (cCoef !== 0)
    terms.push(
      cCoef === 1
        ...
        : cCoef === -1
        ? "-x"
        : `${cCoef > 0 ? "+" : ""}${formatCoeff(cCoef)}x`
    );
  if (d !== 0) terms.push(d > 0 ? `+${formatCoeff(d)}` : formatCoeff(d));
  const expr = terms.length ? terms.join("").replace(/^\+/, "") : "0";
  const stepLines = [
    "三次関数を f(x) = ax³ + bx² + cx + d とおきます。",
    "f'(x) = 3ax² + 2bx + c です。",
  ];
  for (const c of c4) {
    ...
    if (c.kind === "f")
      stepLines.push(
        `f(${xD}) = ${c.y} より ${formatCoeff(c.x ** 3)}a + ${formatCoeff(
          c.x * c.x
        )}b + ${formatCoeff(c.x)}c + d = ${c.y}`
      );
    else
      stepLines.push(
        `f'(${xD}) = ${c.y} より ${formatCoeff(3 * c.x * c.x)}a + ${formatCoeff(
          2 * c.x
        )}b + c = ${c.y}`
      );
  }
  stepLines.push(
    `これを解いて a = ${formatCoeff(a)}, b = ${formatCoeff(
      b
    )}, c = ${formatCoeff(cCoef)}, d = ${formatCoeff(d)}`
  );
  stepLines.push(`答: f(x) = ${expr}`);
  return { expr, steps: stepLines };
}
...
export function solveIntegral(exprStr: string): {
  result: string;
  steps: string[];
} {
  ...
  for (const { coef, power } of terms) {
    if (power === 0) {
      ...
      stepParts.push(
        `∫${termToDisplay(coef, power)} dx = ${termToDisplay(
          coef,
          power
        )} → ${termToDisplay(newCoef, newPower)}（公式 ∫xⁿ dx = xⁿ⁺¹/(n+1)）`
      );
    }
  }
...
function solveDefiniteIntegral(
  ...
  try {
    ...
    const steps: string[] = [
      `与式: ∫[${formatXForStep(a)}, ${formatXForStep(
        b
      )}] (${exprStr.trim()}) dx`,
      "公式 ∫xⁿ dx = xⁿ⁺¹/(n+1) を用いて原始関数を求めます。",
      ...stepParts,
      `原始関数: F(x) = ${Fdisplay}`,
      `定積分 = F(${formatXForStep(b)}) - F(${formatXForStep(
        a
      )}) = ${formatCoeff(Fb)} - ${formatCoeff(Fa)} = ${formatCoeff(value)}`,
      `答: ${formatCoeff(value)}`,
    ];
    return { value, steps };
  } catch {
...
function solveIntegralWithCondition(
  exprStr: string,
  x0: number,
  y0: number
  ...
  try {
    ...
    const steps: string[] = [
      `与式: ∫(${exprStr.trim()}) dx で、F(${formatXForStep(
        x0
      )}) = ${y0} を満たす F(x) を求めます。`,
      "公式 ∫xⁿ dx = xⁿ⁺¹/(n+1) より、原始関数は F(x) = " +
        Fdisplay +
        " + C（Cは積分定数）",
      ...stepParts,
      `F(${formatXForStep(x0)}) = ${y0} より ${formatCoeff(
        Fx0
      )} + C = ${y0} → C = ${formatCoeff(C)}`,
      `答: F(x) = ${result}`,
    ];
    return { result, steps };
  } catch {
...
function cleanExpr(expr: string): string {
  ...
  if (s.includes("=")) {
    ...
    const withExpr = candidates.find(
      (c) => /[x\d⁴³²^]/.test(c) && /^[x\d⁴³²^+\-\s・()]*[x\d⁴³²]/.test(c)
    );
...
type QuadraticCondition =
  | { kind: "f"; x: number; y: number }
  | { kind: "fPrime"; x: number; y: number };
...
function extractQuadraticConditions(problem: string): QuadraticCondition[] {
  ...
  while ((m = fPrimeRegex.exec(t)) !== null) {
    const x = parseXValue(m[1]);
    if (!Number.isNaN(x))
      conditions.push({ kind: "fPrime", x, y: Number(m[2]) });
  }
  // f'(2)を6満たす, f'(2)、6 など（＝がなく値だけ続く表記）
  const fPrimeAltRegex = new RegExp(
    "f'\\s*[（(]\\s*" + xArg + "\\s*[）)]\\s*[を、，,s]*\\s*(-?\\d+)",
    "g"
  );
...
function inferMissingQuadraticCondition(
  ...
  if (hasFPrime2 && !hasFPrime2Value) {
    ...
    return {
      ...
      inferredNote:
        "問題文に f'(2) の値が書かれていなかったため、f'(2)=6 として解いています。",
    };
  }
  return { conditions };
}
...
function parseProblem(problem: string): {
  type:
    | "diff"
    | "integral"
    | "definiteIntegral"
    | "integralWithCondition"
    | "findQuadratic"
    | "findCubic";
  expr?: string;
  atPoint?: number;
  ...
  const looksPoly =
    /求めて|求めよ/i.test(t) &&
    /二次関数|2次関数|二時間数|２次関数|三次関数|3次関数|３次関数|関数\s*f\s*[（(]\s*x\s*[）)]|f\s*[（(]\s*x\s*[）)]\s*を/i.test(
      t
    );
  ...
  if (conditions.length === 2 && looksPoly && !/三次|3次|３次/i.test(t)) {
    const { conditions: c3, inferredNote } = inferMissingQuadraticCondition(
      problem,
      conditions
    );
  ...
  const integralMatch =
    t.match(/∫\s*([^d]+)\s*dx|積分[せ求].*?([x\d⁴³²^+\-\s・]+)/i) ??
    t.match(/([x\d⁴³²^+\-\s・]+)\s*を?\s*積分/);
  if (integralMatch) {
    const raw = (integralMatch[1] ?? integralMatch[2] ?? "")
      .trim()
      .replace(/\s+/g, "");
  ...
  if (
    /接線の傾き|接線.*傾き|傾き.*接線/i.test(t) &&
    /f\s*[（(]\s*x\s*[）)]\s*[=＝]/.test(t)
  ) {
    const fMatch = t.match(
      /f\s*[（(]\s*x\s*[）)]\s*[=＝]\s*([x\d⁴³²^+\-\s・()]+)/
    );
    if (fMatch) {
  ...
  const diffMatch =
    t.match(
      /([x\d⁴³²^+\-\s・/=()]+)\s*を?\s*微分|微分[せ求].*?([x\d⁴³²^+\-\s・/=()]+)/i
    ) ?? t.match(/([x\d⁴³²^+\-\s・/=()]+)\s*の?\s*微分/);
  if (diffMatch) {
  ...
  if (/微分/i.test(t)) {
    const raw =
      t
        ...
        .replace(/\s*[をの]?\s*微分.*$/, "")
        .trim()
        .replace(/\s+/g, "") || t.replace(/\s+/g, "").slice(0, 80);
    const expr = cleanExpr(raw);
    if (expr) {
      const atPoint = extractAtPoint(problem);
      return { type: "diff", expr, ...(atPoint != null ? { atPoint } : {}) };
  ...
  if (/積分/i.test(t) || /∫/.test(t)) {
    ...
    const boundsMatch = t.match(
      /(\d+)\s*から\s*(\d+)\s*まで|∫\s*[_\[]\s*(\d+)\s*[^\d]*\s*(\d+)\s*[\]^]|(\d+)\s*[〜～]\s*(\d+)|x\s*[=＝]\s*(\d+)\s*から\s*x\s*[=＝]\s*(\d+)/
    );
    if (boundsMatch) {
      const a = Number(
        boundsMatch[1] ?? boundsMatch[3] ?? boundsMatch[5] ?? boundsMatch[7]
      );
      const b = Number(
        boundsMatch[2] ?? boundsMatch[4] ?? boundsMatch[6] ?? boundsMatch[8]
      );
      if (!Number.isNaN(a) && !Number.isNaN(b))
        return { type: "definiteIntegral", expr, a, b };
    }
    // 原始関数の条件：F(1)=0、F(1)=0を満たす、x=1のとき0、Cを求めよ（1条件）
    const condMatch = t.match(
      /[Ff]\s*[（(]\s*(-?\d+)\s*[）)]\s*[=＝]\s*(-?\d+)|x\s*[=＝]\s*(\d+)\s*の?\s*とき\s*(-?\d+)/
    );
    if (condMatch) {
      const x0 = Number(condMatch[1] ?? condMatch[3]);
      const y0 = Number(condMatch[2] ?? condMatch[4]);
      if (!Number.isNaN(x0) && !Number.isNaN(y0))
        return { type: "integralWithCondition", expr, x0, y0 };
    }
...
export function explain(problem: string): string {
  ...
  if (type === "findQuadratic" && parsed.conditions) {
    ...
    if (!out)
      return "条件から二次関数を一意に決められませんでした。f(数字)=数字 と f'(数字)=数字 を合わせて3つ、はっきり書いてください。";
    const note = parsed.inferredNote ? parsed.inferredNote + "\n\n" : "";
    return (
      "【二次関数の決定】\n\n" +
      note +
      out.steps.join("\n\n") +
      "\n\nこのアプリの「自分で関数を指定」ページで f(x) = " +
      out.expr.replace(/²/g, "^2").replace(/³/g, "^3") +
      " と入力すると、グラフを確認できます。"
    );
  }
  if (type === "findCubic" && parsed.conditions) {
    ...
    if (!out)
      return "条件から三次関数を一意に決められませんでした。f(数字)=数字 と f'(数字)=数字 を合わせて4つ、はっきり書いてください。";
    return (
      "【三次関数の決定】\n\n" +
      out.steps.join("\n\n") +
      "\n\nこのアプリの「自分で関数を指定」ページで f(x) = " +
  ...
  if (
    ...
    return (
      "【定積分の解説】\n\n" +
      out.steps.join("\n\n") +
      "\n\nこのアプリの「自分で導関数を指定」ページで f'(x) = " +
      parsed.expr +
      " と入力すると、積分された f(x) のグラフを確認できます。"
    );
  }
  if (
    type === "integralWithCondition" &&
    parsed.expr != null &&
    parsed.x0 != null &&
    parsed.y0 != null
  ) {
  ...
  if (type === "diff") {
    ...
    const isTangentSlope =
      /接線の傾き|接線.*傾き/i.test(problem) && atPoint != null;
    ...
    return (
      ...
      "\n\nこのアプリの「自分で関数を指定」ページで f(x) = " +
      expr +
      " と入力すると、f(x) と f'(x) のグラフを同時に確認できます。"
    );
  }
  if (type === "integral") {
